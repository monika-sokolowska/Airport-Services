...
const PushbackForm = ({ flight }) => {
  ...
  useEffect(() => {
    ...
    if (availableEmployees) {
      const serviceEmployees = availableEmployees.filter(
        (availableEmployee) => availableEmployee.service === "Pushback Service"
      );
  ...
  return (
    <form className="service-form" onSubmit={onSubmit}>
      <label className="service-title">Pushback service</label>
      <div>
        ...
        <div className="button-container">
          <input
            value="Previous"
            className="prev-btn"
            onClick={() => dispatch(changeServiceAction("NAVIGATOR"))}
          />
          <input
            value="Next"
            className="next-btn"
            onClick={() => dispatch(changeServiceAction("LUGGAGE_ARRIVAL"))}
            disabled={true}
